...
INNER JOIN t_user_account tr on t_personne_avoir_compte.fk_compte = tr.ID_account"""
                if id_film_sel == 0:
                    ...
                    strsql_genres_films_afficher_data += """ HAVING ID_Person= %(value_id_film_selected)s"""
...
def edit_compte_personne_selected():
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                strsql_genres_afficher = """SELECT ID_role, Role_name FROM t_role ORDER BY ID_role ASC"""
            ...
            lst_data_film_selected = [item['ID_Person'] for item in data_genre_film_selected]
            ...
            lst_data_genres_films_non_attribues = [item['ID_role'] for item in data_genres_films_non_attribues]
            ...
            lst_data_genres_films_old_attribues = [item['ID_role'] for item in data_genres_films_attribues]
            ...
            lst_data_genres_films_non_attribues = [item['Role_name'] for item in data_genres_films_non_attribues]
...
def update_compte_personne_selected():
    if request.method == "POST":
        try:
            ...
            strsql_insert_genre_film = """INSERT INTO t_personne_avoir_role (id_personne_avoir_role,fk_role, fk_Personne)
            ...
            strsql_delete_genre_film = """DELETE FROM t_personne_avoir_role WHERE fk_role = %(value_fk_genre)s AND fk_Personne = %(value_fk_film)s"""
...
def genres_films_afficher_data(valeur_id_film_selected_dict):
    ...
    try:
        ...
        strsql_film_selected = """SELECT ID_Person, First_name, Last_name, Birth_date, gender, GROUP_CONCAT(ID_role) as GenresFilms FROM t_personne_avoir_role
                                        INNER JOIN t_person ON t_person.ID_Person = t_personne_avoir_role.fk_Personne
                                        INNER JOIN t_role ON t_role.ID_role = t_personne_avoir_role.fk_role
                                        WHERE ID_Person = %(value_id_film_selected)s"""

        strsql_genres_films_non_attribues = """SELECT ID_role, Role_name FROM t_role WHERE ID_role not in(SELECT ID_role as idGenresFilms FROM t_personne_avoir_role
                                                    INNER JOIN t_person ON t_person.ID_Person = t_personne_avoir_role.fk_Personne
                                                    INNER JOIN t_role ON t_role.ID_role = t_personne_avoir_role.fk_role
                                                    WHERE ID_Person = %(value_id_film_selected)s)"""

        strsql_genres_films_attribues = """SELECT ID_Person, ID_role, Role_name FROM t_personne_avoir_role
                                            INNER JOIN t_person ON t_person.ID_Person = t_personne_avoir_role.fk_Personne
                                            INNER JOIN t_role ON t_role.ID_role = t_personne_avoir_role.fk_role
                                            WHERE ID_Person = %(value_id_film_selected)s"""
